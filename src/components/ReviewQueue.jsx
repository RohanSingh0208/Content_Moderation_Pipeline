...
const ReviewQueue = ({ queue, setReviewQueue, addToLog }) => {
  ...
  const handleAction = (item, decision) => {
    // Add to log
    addToLog({
      ...
      decidedBy: `Human Reviewer${notes[item.id] ? ` (Note: ${notes[item.id]})` : ''}`,
      flaggedCategory: "multiple/manual"
    });

    // Remove from queue
    setReviewQueue(queue.filter(q => q.id !== item.id));
            ...
            <div key={item.id} className="card" style={{display: 'flex', gap: '2rem'}}>
              <div style={{flex: 2}}>
                ...
                <div style={{backgroundColor: 'var(--bg-light)', padding: '1rem', borderRadius: '6px', marginBottom: '1rem', fontSize: '0.95rem', lineHeight: '1.5'}}>
                  ...
                  {item.content.split(item.flagged_segment).map((part, i, arr) => (
